...
from vllm.attention.ops.RelayAttention import schedule_single_sys, RelayAttentionPlus
...
class RelayAttentionBackend(AttentionBackend):
    ...
    @staticmethod
    def get_name() -> str:
        logger.info('[zhixin] RelayAttentionBackend is created.')
        return "Relay_ATTN"
...
class RelayAttentionMetadata(AttentionMetadata):
    ...
    seqlen4ra: Optional[List[torch.Tensor]] = None
    block_tables4ra: Optional[List[torch.Tensor]] = None
    ...
    def decode_metadata(self) -> Optional["RelayAttentionMetadata"]:
        ...
        # Prepare attn metadata for RelayAttention
        with Timer("RelayAttn schedule", sync_gpu=False, disable=True):
            # print(f"seq_lens: {self.seq_lens[self.num_prefills:]}")
            # print(f"block_tables: {block_tables}")
            seqlen4ra, block_tables4ra = schedule_single_sys(
        ...
        self._cached_decode_metadata = RelayAttentionMetadata(
            ...
            cross_slot_mapping=self.cross_slot_mapping,
            cross_block_tables=self.cross_block_tables)
        self._cached_decode_metadata.seqlen4ra = seqlen4ra
        self._cached_decode_metadata.block_tables4ra = block_tables4ra
        return self._cached_decode_metadata
...
class RelayAttentionImpl(AttentionImpl):
    ...
    def forward(
        ...
        if decode_meta := attn_metadata.decode_metadata:
            ...
            # print(f"seqlen4ra: {decode_meta.seqlen4ra}")
            # print(f"block_tables4ra: {decode_meta.block_tables4ra}")
            # print(f"block_table: {decode_meta.block_tables}")
            # if decode_meta.seq_lens is not None:
            #     print(f"seqlen: {decode_meta.seq_lens[num_prefill_query_tokens:]}")
            output[num_prefill_query_tokens:] = RelayAttentionPlus(
                q=decode_query.unsqueeze(1),
                k_cache_paged=key_cache,
                v_cache_paged=value_cache,
                seqlens=decode_meta.seqlen4ra,
                block_tables=decode_meta.block_tables4ra,
            )
